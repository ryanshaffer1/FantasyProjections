    ...
    def gather_historical_odds(self, odds_df=None, game_times=None):
        ...
        for game_time in game_times:
            ...
            for market in market_data:
                ...
                odds_df = pd.concat((odds_df, single_market_odds_df))

        return odds_df

    def reformat_odds_df(self, df_to_format):
        ...
        df_to_format["Year"] = self.year
        df_to_format["Week"] = self.week
        df_to_format["Game ID"] = self.game_id

        # Add Player ID based on roster
        roster_inds = df_to_format["Player Name"].apply(find_matching_name_ind, args=(self.roster_df["Player Name"].unique(),))
        df_to_format["Player ID"] = roster_inds.apply(
            lambda x: self.roster_df.reset_index().iloc[int(x)][PRIMARY_PLAYER_ID] if not np.isnan(x) else np.nan,
        )
        ...
        df_to_format = df_to_format.set_index("Player ID")
