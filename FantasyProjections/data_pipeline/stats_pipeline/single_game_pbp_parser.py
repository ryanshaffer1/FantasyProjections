...
class SingleGamePbpParser(SingleGameDataWorker):
    ...
    def parse_play_by_play(self, game_times="all"):
        ...
        list_of_player_dfs = self.roster_df.reset_index().apply(self.__parse_player_stats, args=(game_times,), axis=1)
        ...
        for stats_df in [midgame_stats_df, final_stats_df]:
            stats_df[["Year", "Week"]] = [self.year, self.week]
            stats_df["Team"] = self.roster_df.loc[(stats_df[PRIMARY_PLAYER_ID], "Team")].tolist()
            stats_df["Opponent"] = self.game_info.set_index("Team Abbrev").loc[stats_df["Team"], "Opp Abbrev"].to_list()
            stats_df["Site"] = self.game_info.set_index("Team Abbrev").loc[stats_df["Team"], "Site"].to_list()
            stats_df[["Team Wins", "Team Losses", "Team Ties"]] = (
                self.game_info.set_index("Team Abbrev")
    ...
    def __parse_player_stats(self, player_info, game_times):
        ...
        # Field Position
        player_stats_df["Field Position"] = self.pbp_df.apply(
    ...
    def __assign_stats_from_plays(self, player_stat_df, player_info, team_abbrev):
        ...
        player_stat_df["Pass Att"] = self.pbp_df.apply(lambda x: (x["passer_player_id"] == player_id) & (x["sack"] == 0), axis=1)
        ...
        player_stat_df["Rush Att"] = self.pbp_df.apply(lambda x: (x["rusher_player_id"] == player_id), axis=1)
        # Rushing Yards
        player_stat_df["Rush Yds"] = self.pbp_df.apply(
