...
# Use low alpha on scatterplot values for larger datasets
LOW_ALPHA_MIN_DATA = 2000
...
def gen_scatterplots(result, **kwargs):
    ...
    if ideal_line:
        if num_subplots > 1:
            fig.legend([ideal_line, bestfit_line], ["Ideal Trendline", "Actual Trendline"], loc="lower right")
        else:
            axs.flat[0].legend([ideal_line, bestfit_line], ["Ideal Trendline", "Actual Trendline"], loc="lower right")
    ...
    if subtitle:
        title += "\n" + subtitle + "".join(["\n " + key + ": " + ", ".join(plot_slice[key]) for key in list(plot_slice.keys())])
...
def scatter_hist(truth_dfs, predict_dfs, ax, column, legend_slice, histograms=True):
    ...
    for x_df, y_df in zip(truth_dfs, predict_dfs):
        x, y = x_df[column], y_df[column]
        # Set alpha based on data size (threshold set empirically by what has looked good)
        alpha = 0.1 if x.shape[0] > LOW_ALPHA_MIN_DATA else 0.5
        alpha = 1
        ...
        lim_vals = np.linspace(
            min(np.concat((ax.get_xlim(), ax.get_ylim()))),
    ...
    if legend_slice:
        leg = ax.legend(
            [", ".join(val) for key in legend_slice for val in legend_slice[key]],
...
def data_slice_to_plot(result, plot_slice, legend_slice, return_lists=True):
    ...
    if plot_slice:
        for key in plot_slice:
            indices_to_keep = id_df.apply(check_df_for_slice, args=(key, plot_slice), axis=1)
...
def plot_game_timeline(result, game_id, fig=None):
    ...
    ax.legend(["Live Fantasy Score", f"{result.predictor_name} Final Fantasy Score", "True Final Fantasy Score"])
