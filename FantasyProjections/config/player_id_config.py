...
def fill_blank_player_ids(players_df, master_id_file=None, pfr_id_filename=None, add_missing_pfr=False, update_master=False):
    ...
    players_df.loc[:,ALT_PLAYER_IDS] = players_df.apply(
        lambda x: id_df.set_index(PRIMARY_PLAYER_ID).loc[x[PRIMARY_PLAYER_ID], ALT_PLAYER_IDS], axis=1)
    ...
    if update_master:
        update_master_player_ids(addl_players_df=players_df,
...
def update_master_player_ids(addl_players_df=None, master_id_file=None, pfr_id_filename=None, add_missing_pfr=False, save_data=False):
...
def __setup_master_id_df(master_id_file, addl_players_df=None):
    ...
    with pd.option_context("future.no_silent_downcasting", True):
        id_df = id_df.groupby(PRIMARY_PLAYER_ID, dropna=False).apply(lambda x: x.bfill().iloc[0], include_groups=False).reset_index()

    return id_df


def __add_missing_pfr_ids(df, pfr_id_filename=None):
    ...
    try:
        with open(pfr_id_filename, 'r', encoding='utf-8') as file:
    ...
    pfr_ids_found = []
    n_missing = df['pfr_id'].isna().sum()
    for ind, (_, row) in enumerate(df.loc[df['pfr_id'].isna()].iterrows()):
    ...
    # Update dataframe
    df.loc[df['pfr_id'].isna(),'pfr_id'] = pfr_ids_found
