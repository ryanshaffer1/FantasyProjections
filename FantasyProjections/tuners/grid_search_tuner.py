...
class GridSearchTuner(HyperParamTuner):
    ...
    def refine_grid(self, ind):
        ...
        for hp in self.param_set.hyper_parameters:
            ...
            if len(hp.gridpoints) > 1:
                scale_factor = 1 / (len(hp.gridpoints) - 1) / 2 if center_val in hp.val_range else 1 / (len(hp.gridpoints) - 1)
        ...
        for tune_layer in range(self.hyper_tuner_layers):
            ...
            if self.optimize_hypers:
                ...
                self._save_hp_tuning_results(
                    addl_columns={"Grid Search Layer": tune_layer},
                    filename=self.save_file,
                    log_name=f"Layer {tune_layer + 1}",
    ...
    def __gen_gridpoints(self, hp):
        ...
        if isinstance(hp.val_range, list):
            match hp.val_scale:
                case "linear":
                    hp.gridpoints = np.interp(
                        range(self.hyper_tuner_steps_per_dim),
                        [0, self.hyper_tuner_steps_per_dim - 1],
                        hp.val_range,
                    ).tolist()
                case "log":
                    hp.gridpoints = (
