...
from neural_net import HyperParameter, HyperParameterSet, NeuralNetwork
...
def check_all_model_layers_changed(model1, model2):
    ...
    if not compare_net_sizes(model1, model2):
        msg = "Models are not the same size/shape"
...
class TestModifyHyperParameterValues_NeuralNetPredictor(unittest.TestCase):
    def setUp(self):
        ...
        self.mini_batch_size_hp = HyperParameter("mini_batch_size", optimizable=False, value=self.mini_batch_size)
        self.learning_rate_hp = HyperParameter("learning_rate", optimizable=False, value=self.learning_rate)
        self.lmbda_hp = HyperParameter("lmbda", optimizable=False, value=self.lmbda)
        self.loss_fn_hp = HyperParameter("loss_fn", optimizable=False, value=self.loss_fn)
        self.linear_stack_hp = HyperParameter("linear_stack", optimizable=False, value=self.linear_stack)
        self.hp_set = HyperParameterSet(
            (self.mini_batch_size_hp, self.learning_rate_hp, self.lmbda_hp, self.loss_fn_hp, self.linear_stack_hp),
        )
        self.hp_dict = {
...
class TestConfigureModelAndOptimizer_NeuralNetPredictor(unittest.TestCase):
    ...
    def test_updated_hp_values_result_in_correct_model_and_optimizer(self):
        ...
        expected_optimizer = torch.optim.SGD(self.predictor.model.parameters(), lr=self.learning_rate, weight_decay=self.lmbda)
...
class TestTrainAndValidate_NeuralNetPredictor(unittest.TestCase):
    def setUp(self):
        ...
        pbp_df_modified = mock_data_predictors.pbp_df_neural_net.copy()
        pbp_df_modified["Rush Yds"] *= 2
        self.validation_data = StatsDataset(
            name="dataset",
            id_df=mock_data_predictors.id_df,
            pbp_df=pbp_df_modified,
            boxscore_df=mock_data_predictors.bs_df,
