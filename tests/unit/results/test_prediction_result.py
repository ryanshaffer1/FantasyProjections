...
class TestConstructor_PredictionResult(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.predicts = self.predictor.eval_truth(eval_data=self.dataset)
    ...
    def test_pbp_with_fantasy_points(self):
        ...
        expected_pbp_result = stats_to_fantasy_points(
            self.dataset.x_data,
            stat_configs=self.dataset.x_data_columns,
            normalized=True,
        )
...
class TestConstructor_PredictionResultGroup(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Predicted and true stats
        self.predicts = self.predictor1.eval_truth(eval_data=self.dataset)
        self.truths = self.predictor1.eval_truth(eval_data=self.dataset)
        # PredictionResult objects
        self.prediction_result1 = PredictionResult(
            dataset=self.dataset,
            predicts=self.predicts,
            truths=self.truths,
            predictor_name=self.predictor1.name,
        )
        self.prediction_result2 = PredictionResult(
            dataset=self.dataset,
            predicts=self.predicts,
            truths=self.truths,
