...
class TestUnNormalizeStat(unittest.TestCase):
    # Set Up
    def setUp(self):
        self.thresholds = {"StatA": [-1, 1], "StatB": [100, 200], "StatC": [-50, 10000]}
        self.column_names = list(self.thresholds.keys())
...
class TestStatsToFantasyPoints(unittest.TestCase):
    # Set Up
    def setUp(self):
        self.stat_configs = {
    ...
    def test_zero_stats_equals_zero_points(self):
        ...
        pdtest.assert_frame_equal(
            proj.stats_to_fantasy_points(zero_df, stat_configs=self.stat_configs),
            zero_result,
            check_dtype=False,
        )
    ...
    def test_one_of_each_stat_equals_correct_points(self):
        pdtest.assert_frame_equal(
            proj.stats_to_fantasy_points(self.one_df, stat_configs=self.stat_configs),
            self.one_result,
            check_dtype=False,
        )
    ...
    def test_negative_one_of_each_stat_equals_correct_points(self):
        ...
        minus_one_result["Fantasy Points"] = self.one_result["Fantasy Points"] * (-1)

        pdtest.assert_frame_equal(
            proj.stats_to_fantasy_points(minus_one_df, stat_configs=self.stat_configs),
            minus_one_result,
            check_dtype=False,
        )
    ...
    def test_tensor_input_with_stat_index(self):
        pdtest.assert_frame_equal(
            proj.stats_to_fantasy_points(self.one_tensor, stat_configs=self.stat_configs),
            self.one_result,
            check_dtype=False,
        )
    ...
    def test_series_input_gives_right_result(self):
        ...
        pdtest.assert_frame_equal(
            proj.stats_to_fantasy_points(one_series, stat_configs=self.stat_configs),
            self.one_result,
            check_dtype=False,
        )

    def test_input_df_with_additional_column_names_gives_no_error(self):
        # Custom setup
        ...
        proj.stats_to_fantasy_points(one_df_addl_column, stat_configs=self.stat_configs)
    ...
    def test_input_df_with_missing_stats_gives_error(self):
        ...
        with self.assertRaises(ValueError):
            proj.stats_to_fantasy_points(one_df_without_stat_a, stat_configs=self.stat_configs)
