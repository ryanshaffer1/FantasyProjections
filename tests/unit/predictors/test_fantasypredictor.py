...
class TestEvalTruth_FantasyPredictor(unittest.TestCase):
    ...
    def test_correct_calculation_of_fantasy_points(self):
        result = self.predictor.eval_truth(self.dataset)
        expected = stats_to_fantasy_points(
            self.dataset.y_data,
    ...
    def test_input_normalized_false(self):
        result = self.predictor.eval_truth(self.dataset, normalized=False)
        expected = stats_to_fantasy_points(
            self.dataset.y_data,
            stat_configs=self.dataset.y_data_columns,
            normalized=False,
...
class TestGenPredictionResult_FantasyPredictor(unittest.TestCase):
    ...
    def test_generation_of_prediction_result(self):
        pred_result = self.predictor._gen_prediction_result(
            self.predicts,
            self.truths,
            self.dataset,
        )
