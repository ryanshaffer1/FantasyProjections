...
class TestEvalModel_LastNPredictor(unittest.TestCase):
    ...
    def test_last_one_game_score_returns_correct_result(self):
        result = self.predictor.eval_model(eval_data=self.dataset, all_data=self.dataset)

        # Get the last game's Fantasy Points by using the hard-coded map between a game and its previous
        stats = stats_to_fantasy_points(
            self.dataset.y_data,
            stat_configs=self.dataset.y_data_columns,
            normalized=True,
        )
        stats["Last Game Index"] = mock_data_predictors.map_to_last_game
        prev_game_score = stats.apply(
            lambda x: stats.loc[x["Last Game Index"], "Fantasy Points"] if x["Last Game Index"] >= 0 else 0,
            axis=1,
        )
    ...
    def test_last_two_games_score_returns_correct_result(self):
        ...
        second_prev_game_score = stats.apply(
            lambda x: stats.loc[x["Last Game Index"], "Fantasy Points"] if x["Last Game Index"] >= 0 else np.nan,
            axis=1,
        )
    ...
    def test_input_normalized_false(self):
        result = self.predictor.eval_model(self.dataset, normalized=False)
    ...
    def test_input_normalized_true(self):
        result = self.predictor.eval_model(self.dataset, normalized=True)
