...
class TestConstructor_StatsDataset(unittest.TestCase):
    ...
    def test_basic_attributes_df_input(self):
        dataset = StatsDataset(name=self.name, id_df=self.id_df, pbp_df=self.pbp_df, boxscore_df=self.bs_df)
    ...
    def test_non_pandas_df_inputs_raises_error(self):
        with self.assertRaises(TypeError):
            StatsDataset(name=self.name, id_df=self.id_df, pbp_df=torch.tensor(self.pbp_df.values), boxscore_df=self.bs_df)
    ...
    def test_slicing_data_by_indices(self):
        i_start = 3
        i_end = 6
        dataset_sliced_by_index = StatsDataset(
            name=self.name,
            id_df=self.id_df,
            pbp_df=self.pbp_df,
            boxscore_df=self.bs_df,
        ...
        dataset_subset_of_data = StatsDataset(
            name=self.name,
    ...
    def test_slicing_data_by_criteria_simple(self):
        player_id = "00-0033699"  # Austin Ekeler
        df_indices = self.id_df["Player ID"] == player_id
        dataset_sliced_by_criteria = StatsDataset(
            name=self.name,
            id_df=self.id_df,
            pbp_df=self.pbp_df,
            boxscore_df=self.bs_df,
    ...
    def test_shuffle(self):
        ...
        dataset_manually_shuffled = StatsDataset(
            name=self.name,
            id_df=self.id_df,
            pbp_df=self.pbp_df,
            boxscore_df=self.bs_df,
...
class TestEquals_StatsDataset(unittest.TestCase):
    ...
    def setUp(self):
        # Dataset 1
        self.dataset = StatsDataset(name="dataset", id_df=mock_data.id_df, pbp_df=mock_data.pbp_df, boxscore_df=mock_data.bs_df)
        # Dataset 2 using same inputs
        self.identical_dataset = StatsDataset(
            name="dataset",
        ...
        self.dataset_new_name = StatsDataset(
            name="foo",
            id_df=mock_data.id_df.copy(),
            pbp_df=mock_data.pbp_df.copy(),
            boxscore_df=mock_data.bs_df.copy(),
...
class TestConcat_StatsDataset(unittest.TestCase):
    ...
    def setUp(self):
        ...
        id_df1 = pd.DataFrame(
            data=[
                ["00-0033699", "Austin Ekeler", 2024, 1, "WAS", "TB", "RB", 0],
                ["00-0033699", "Austin Ekeler", 2024, 1, "WAS", "TB", "RB", 1],
                ["00-0033699", "Austin Ekeler", 2024, 1, "WAS", "TB", "RB", 2],
                ["00-0033699", "Austin Ekeler", 2024, 1, "WAS", "TB", "RB", 3],
            ],
            columns=["Player ID", "Player Name", "Year", "Week", "Team", "Opponent", "Position", "Elapsed Time"],
        )
        pbp_df1 = pd.DataFrame(
            data=[[0, 0.65], [0.016666667, 0.34], [0.033333333, 0.55], [0.05, 0.6]],
            columns=["Elapsed Time", "Field Position"],
        )
        bs_df1 = pd.DataFrame(
            data=[[0, 0, 0.047619048], [0, 0, 0.047619048], [0, 0, 0.047619048], [0, 0, 0.047619048]],
        ...
        id_df2 = pd.DataFrame(
            data=[
                ["00-0035208", "Olamide Zaccheaus", 2024, 4, "WAS", "ARI", "WR", 22],
                ["00-0035208", "Olamide Zaccheaus", 2024, 4, "WAS", "ARI", "WR", 23],
                ["00-0035208", "Olamide Zaccheaus", 2024, 4, "WAS", "ARI", "WR", 24],
            ],
            columns=["Player ID", "Player Name", "Year", "Week", "Team", "Opponent", "Position", "Elapsed Time"],
        )
        pbp_df2 = pd.DataFrame(
            data=[[0.366666667, 0.71], [0.383333333, 0.55], [0.4, 0.18]],
            columns=["Elapsed Time", "Field Position"],
        )
        bs_df2 = pd.DataFrame(
            data=[[0, 0, 0.047619048], [0, 0, 0.047619048], [0, 0, 0.047619048]],
            columns=["Pass Att", "Pass Cmp", "Pass Yds"],
        )
    ...
    def test_concat_not_inplace_returns_correct_result(self):
        self.assertTrue(self.comb_dataset.equals(self.dataset1.concat(self.dataset2, inplace=False)))
...
class TestCopy_StatsDataset(unittest.TestCase):
    ...
    def setUp(self):
        self.name = "dataset"
        self.dataset = StatsDataset(name=self.name, id_df=mock_data.id_df, pbp_df=mock_data.pbp_df, boxscore_df=mock_data.bs_df)
...
class TestSliceByCriteria_StatsDataset(unittest.TestCase):
    ...
    def test_slice_by_player_id(self):
        player_ids = ["00-0035208"]  # Olamide Zaccheaus
        ...
        dataset_sliced = self.dataset.slice_by_criteria(inplace=False, player_ids=player_ids)
...
class TestRemoveGameDuplicates_StatsDataset(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.dummy_dataset = StatsDataset(name="dataset", id_df=id_df, pbp_df=pbp_df, boxscore_df=bs_df)

        # Desired result: dataset with a single row per player/game
        unique_game_indices = np.logical_not(id_df.duplicated(subset=("Player ID", "Year", "Week"), keep="first"))
        ...
        self.dataset_no_dupe_games = StatsDataset(
            name="dataset",
