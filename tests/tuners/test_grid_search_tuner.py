...
class TestConstructor_GridSearchTuner(unittest.TestCase):
    ...
    def test_gridpoints_and_value_combos_hyperparameter_with_invalid_val_scale_defaults_to_selection(
        self,
    ):
        hp3 = HyperParameter(
        ...
        expected_values = {
            "hp1": [4] * expected_combinations,
            "hp2": [-2.5, -2, -1.5, -1, -0.5, -2.5, -2, -1.5, -1, -0.5],
            "hp3": [1, 1, 1, 1, 1, 10, 10, 10, 10, 10],
        }
...
class TestTuneHyperParameters_GridSearchTuner(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.eval_function = (
            lambda param_set: param_set.get("hp1").value
            + 5 * param_set.get("hp2").value
            + np.sqrt(param_set.get("hp3").value)
        )
        self.max_val = 101.5
        self.min_val = -7.5
    ...
    def test_finds_correct_value_to_zoom_in_on(self):
        def eval_func(param_set):
            return (param_set.get("hp2").value - (-0.5)) ** 2 + (
                param_set.get("hp3").value - np.sqrt(10)
            ) ** 2

        min_val = self.tuner.tune_hyper_parameters(eval_function=eval_func)

        expected_min_val = (
            0  # Will only be found by the computer if it refines grid correctly.
        )
    ...
    def test_perf_list_and_table_are_correct_length_for_one_tune_layer(self):
        ...
        expected_length = (
            self.settings["hyper_tuner_steps_per_dim"] ** 2
            * self.tuner.hyper_tuner_layers
        )
...
class TestRefineGrid_GridSearchTuner(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.eval_function = (
            lambda param_set: (param_set.get("hp2").value - (-0.5)) ** 2
            + (param_set.get("hp3").value - np.sqrt(10)) ** 2
        )
        self.max_val = 101.5
        self.min_val = -7.5
    ...
    def test_refines_to_correct_linear_and_log_gridpoints_and_values(self):
        ...
        expected_values = {
            "hp1": [4] * self.tuner.n_value_combinations,
            "hp2": [-1, -0.75, -0.5, -1, -0.75, -0.5, -1, -0.75, -0.5],
            "hp3": [1, 1, 1, np.sqrt(10), np.sqrt(10), np.sqrt(10), 10, 10, 10],
        }
    ...
    def test_refines_to_center_of_grid_correctly(self):
        ...
        expected_values = {
            "hp1": [4] * self.tuner.n_value_combinations,
            "hp2": [-2, -1.5, -1, -2, -1.5, -1, -2, -1.5, -1],
            "hp3": [10, 10, 10, 100, 100, 100, 1000, 1000, 1000],
        }
