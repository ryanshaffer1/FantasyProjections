...
class TestConstructor_RecRandomSearchTuner(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.expected_gridpoints = {"hp1": [4], "hp2": [-2.5, -2, -1.5, -1, -0.5], "hp3": [1, 10, 100, 1000, 10000]}
        ...
        self.def_r_percentile = calc_r_percentile(self.default_settings["n_explore_samples"], self.default_settings["p_conf"])
        ...
        self.custom_r_percentile = calc_r_percentile(self.custom_settings["n_explore_samples"], self.custom_settings["p_conf"])
    ...
    def test_basic_attributes_optimizing_no_optional_inputs(self):
        tuner = RecursiveRandomSearchTuner(self.hp_set, save_file=self.save_file, **self.base_settings)
    ...
    def test_basic_attributes_all_optional_inputs(self):
        tuner = RecursiveRandomSearchTuner(self.hp_set, save_file=self.save_file, **self.base_settings, **self.custom_settings)
    ...
    def test_r_percentile_input_overrides_n_explore_samples(self):
        ...
        expected_n = calc_n_samples(self.custom_settings["r_percentile"], self.custom_settings["p_conf"])
    ...
    def test_v_expect_imp_input_overrides_l_exploit_samples(self):
        ...
        del self.custom_settings["max_samples"]
        expected_l = calc_n_samples(self.custom_settings["v_expect_imp"], self.custom_settings["q_conf"])
    ...
    def test_hp_values_optimize_true(self):
        tuner = RecursiveRandomSearchTuner(self.hp_set, save_file=self.save_file, **self.base_settings, **self.default_settings)
...
class TestTuneHyperParameters_RecRandomSearchTuner(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.eval_function = (
            lambda param_set: param_set.get("hp1").value + 5 * param_set.get("hp2").value + np.sqrt(param_set.get("hp3").value)
        )
    ...
    def test_perf_list_and_table_are_correct_length_exploration_only(self):
        ...
        self.tuner.tune_hyper_parameters(
    ...
    def test_save_function_and_reset_function_handles_behave_correctly(self):
        ...
        self.tuner.tune_hyper_parameters(eval_function=self.eval_function, save_function=test_save, reset_function=test_reset)
    ...
    def test_improper_function_kwargs_raise_error(self):
        ...
        with self.assertRaises(TypeError):
            self.tuner.tune_hyper_parameters(eval_function=test_eval, eval_kwargs={"a": a_val, "b": b_val})
