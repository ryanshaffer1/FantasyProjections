...
class TestSaveHPTuningResults_HyperParamTuner(unittest.TestCase):
    ...
    def test_save_without_perf_data_returns_empty_dataframe(self):
        ...
        save_df = self.tuner._save_hp_tuning_results()
        empty_df = pd.DataFrame(columns=self.df_columns)

        pdtest.assert_frame_equal(save_df, empty_df)

    def test_save_with_perf_data_returns_populated_dataframe(self):
        result_df = self.tuner._save_hp_tuning_results()

        expected_df = pd.DataFrame(
            data=[self.hp1.values, self.hp2.values, self.tuner.perf_list],
            index=self.df_columns,
        ).transpose()

        pdtest.assert_frame_equal(result_df, expected_df, check_dtype=False)

    def test_save_additional_columns_with_list_of_data(self):
        addl_data = [10, 20, 30, 40]
        self.df_columns.append("foo")
        result_df = self.tuner._save_hp_tuning_results(addl_columns={"foo": addl_data})

        expected_df = pd.DataFrame(
            data=[self.hp1.values, self.hp2.values, self.tuner.perf_list, addl_data],
            index=self.df_columns,
        ).transpose()
    ...
    def test_save_additional_columns_with_single_number_works(self):
        ...
        expected_df = pd.DataFrame(
            data=[self.hp1.values, self.hp2.values, self.tuner.perf_list, addl_data_list],
            index=self.df_columns,
        ).transpose()
    ...
    def test_save_to_file_works_as_expected(self):
        ...
        result_df = pd.read_csv(self.save_file, index_col=0)
    ...
    def tearDown(self):
        ...
        for filename in Path(self.save_folder).iterdir():
