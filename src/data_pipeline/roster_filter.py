...
def generate_roster_filter(input_dfs, final_stats_df, save_file=None, num_players=300, plot_filter=False):
    """Generates a short list of players to focus data collection on, based on highest average Fantasy Points per game.
        ...
        Args:
            input_dfs (list): List of tuples of DataFrames. Each element in the list corresponds to a year. Each tuple for a given year contains two elements:
                - play-by-play DataFrame (raw data, not parsed into player stats) from nfl-verse
                - weekly NFL rosters DataFrame from nfl-verse
                Note that play-by-play DataFrame is not accessed, but this input_dfs data structure is generated by another function and simply input here unmodified.
